                    ...
                    <td>
                        <div class="dropdown">        <!-- pembungkus dropdown -->   {{-- bikin titik 3 --}}
                            ...
                            <ul class="dropdown-menu">          <!-- isi menu dropdown (edit, hapus) -->
                                <li>
                                    <a href="{{ route('halaman.edit', $item->id) }}" class="dropdown-item">
                                        <i class="bi bi-pencil-square me-2"></i>Edit
                                    </a>
                                </li>
                                <li>
                                <form onsubmit="return confirm('Are you sure you want to delete {{ $item->judul }} ?')" action="{{ route('halaman.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <form onsubmit="return confirm('Apakah Anda yakin ingin menghapus data {{ $item->judul }} ?')" action="{{ route('halaman.destroy', $item->id) }}" class="d-inline" method="POST">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-sm btn-danger"
                            type="submit" name="submit">Hapus</button>
                        </form> --}}
                    </td>
                </tr>
                <?php $i++; ?>
